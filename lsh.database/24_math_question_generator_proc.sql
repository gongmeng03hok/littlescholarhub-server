-- ============================================================
--  Little Scholars Hub -- Migration 024
--  usp_GenerateMathQuestion: a genuine SQL-side dynamic question
--  generator. Given @grade (and optionally @topic), computes a
--  randomised, correct math question + answer + 3 distractors
--  entirely in T-SQL (no hardcoded question bank) and returns one
--  row. Called repeatedly by the API to build a practice set.
--
--  New topics beyond the existing Python generator: place value,
--  rounding, comparing numbers, counting money, adding decimals,
--  order of operations, factors/primes, elapsed time.
--
--  NOTE: string literals are kept plain-ASCII on purpose (avoid
--  the multiplication sign / em-dash) since non-ASCII characters
--  in this file were getting mangled on the way into SQL Server
--  through some client tool paths.
-- ============================================================

USE LittleScholarHub;
GO

CREATE OR ALTER PROCEDURE dbo.usp_GenerateMathQuestion
    @grade TINYINT = 2,
...
BEGIN
    ...
    BEGIN
        ...
        SET @question = N'In the number ' + @pv_str + N', what is the value of the digit in the ' + @pv_place_name + N' place?';
